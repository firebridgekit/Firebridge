...
const checkUserAccess = (permissions: UserPermissions, requiredScope: string): boolean => {
  ...
  if (permissions.role === 'moderator' && requiredScope.startsWith('content:')) {
    return true;
  }
...
console.log('Admin can moderate content:', checkUserAccess(adminUser, 'content:moderate'));
console.log('Regular user can moderate content:', checkUserAccess(regularUser, 'content:moderate'));
console.log('Moderator can moderate content:', checkUserAccess(moderatorUser, 'content:moderate'));

// Example 3: Using PlatformError for standardized error handling
const createPlatformError = (message: string, code: BaseErrorCode): PlatformError => ({
  type: 'PlatformError',
  message,
  code,
...
type AppErrorCode = BaseErrorCode | 'INVALID_INPUT' | 'USER_SUSPENDED' | 'RATE_LIMITED';
...
const processSerializedTimestamp = (timestamp: SerializedFirestoreTimestamp): Date | null => {
...
console.log('Serialized timestamp:', processSerializedTimestamp(serializedTimestamp));
...
const updateArticle = (article: ArticleWithMetadata, updates: Partial<Article>): ArticleWithMetadata => {
  const now: FirestoreTimestamp = {
    seconds: Math.floor(Date.now() / 1000),
    nanoseconds: (Date.now() % 1000) * 1000000
  };
...
const processCompleteUser = (user: CompleteUser) => {
  ...
  console.log(`Last updated: ${user.metadata.timeUpdated ? processTimestamp(user.metadata.timeUpdated) : 'Never'}`);
