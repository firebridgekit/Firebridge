...
We can also use dependencies to build our own custom hooks. This will make for a great developer experience when we go to build our book page.
...
const useOwnReviewForBook = () =>
  useDocument<BookReview>(uid =>
    // Reviews are stored at `books/{bookId}/reviews/{userId}`.
    firestore.collection('books').doc(BOOK_ID).collection('reviews').doc(uid),
  )
```
